...
async function runExtraction(invoiceId, buffer, hint, chatId = null) {
  ...
  try {
    ...
    // Goods purchases: tag lines as stock movements and auto-match each to a real
    // Tally stock item so the item-invoice builder fires. Weak matches are flagged
    // (needs_match) for the reviewer to fix via the stock-item dropdown.
    if (inv.doc_type === 'purchase_invoice') {
      await attachStockMatches(inv);
    }
...
// ── Stock-item auto-matching ──────────────────────────────────────────────
// A supplier invoice line ("TANT CAP 1UF 35V 1206") rarely equals the Tally
// stock-item name ("SMD Tan Cap : 1uF 35v 1206 (A)"). We match by shared
// distinctive tokens — value tokens (those containing a digit: 1UF, 35V, 1206)
// are the real discriminators, so they count double. The review-screen dropdown
// is the fallback for anything that doesn't auto-match confidently.
const STOCK_MATCH_MIN_SCORE = 3;   // need at least this weighted token overlap
const _STOP_TOKENS = new Set(['THE','AND','FOR','PCS','NOS','QTY','SMD','CAP']);

function _stockTokens(s) {
  return String(s || '')
    .toUpperCase()
    .replace(/[^A-Z0-9.±%]/g, ' ')
    .split(/\s+/)
    .filter(t => t && t.length >= 2 && !_STOP_TOKENS.has(t));
}

function _scoreStockMatch(descTokens, itemName) {
  const itemTokens = new Set(_stockTokens(itemName));
  let score = 0;
  for (const t of descTokens) {
    if (itemTokens.has(t)) score += /\d/.test(t) ? 2 : 1;
  }
  return score;
}

// Returns { name, score } of the best Tally stock item, or null if nothing
// clears the confidence threshold. hsn match adds a point.
function matchStockItem(description, hsn, stockItems) {
  const descTokens = _stockTokens(description);
  if (!descTokens.length) return null;
  let best = null, bestScore = 0;
  for (const it of stockItems) {
    let s = _scoreStockMatch(descTokens, it.name);
    if (hsn && it.hsn_code && String(it.hsn_code).trim() === String(hsn).trim()) s += 1;
    if (s > bestScore) { bestScore = s; best = it; }
  }
  if (!best || bestScore < STOCK_MATCH_MIN_SCORE) return null;
  return { name: best.name, score: bestScore };
}

// Mutates inv.line_items in place: marks goods lines kind:'stock' and pre-fills
// stock_item_name from the best Tally match. Lines that don't match confidently
// keep an empty stock_item_name + needs_match:true so the reviewer can pick one.
// A line already carrying a stock_item_name (e.g. user-corrected) is left alone.
async function attachStockMatches(inv) {
  const items = Array.isArray(inv.line_items) ? inv.line_items : [];
  if (!items.length) return;
  let stockItems = [];
  try { stockItems = await queryAll('SELECT name, parent, hsn_code FROM crm_tally_stock_items'); }
  catch { stockItems = []; }
  inv.line_items = items.map(it => {
    if (it.stock_item_name) {                       // already matched/confirmed — leave it
      return { ...it, kind: 'stock', needs_match: false };
    }
    const m = stockItems.length ? matchStockItem(it.description, it.hsn, stockItems) : null;
    return {
      ...it,
      kind: 'stock',
      stock_item_name: m ? m.name : '',
      stock_match_score: m ? m.score : 0,
      needs_match: !m,
    };
  });
}
...
// Godown / batch a stock purchase lands in. Tally's single-location default
// names; override here if your company uses different ones.
const GODOWN_DEFAULT = 'Main Location';
const BATCH_DEFAULT  = 'Primary Batch';

// Inventory entry: a stock line inside an item-invoice Purchase voucher.
// Mirrors Tally's OWN export of a goods purchase (verified by live test-push):
//   • AMOUNT is NEGATIVE (Tally's purchase sign convention: debit/deemed-positive)
//   • qty formatted " 2000.00 pcs" (leading space, 2 decimals), rate "3.10/pcs"
//   • a BATCHALLOCATIONS block (godown + batch) — required for stock movement
//   • a nested ACCOUNTINGALLOCATIONS that debits the purchase ledger
function inventoryEntry(stockName, qty, rate, amount, accLedger, unit) {
  const unitStr = unit || 'pcs';
  const q       = n(qty) || 0;
  const qtyStr  = ` ${q.toFixed(2)} ${unitStr}`;          // " 10.00 pcs"
  const negAmt  = `-${Math.abs(n(amount)).toFixed(2)}`;   // purchase → negative
  const rateStr = rate ? `${n(rate).toFixed(2)}/${unitStr}` : '';
  return `
            <ALLINVENTORYENTRIES.LIST>
              ...
              <BATCHALLOCATIONS.LIST>
                <GODOWNNAME>${xmlEscape(GODOWN_DEFAULT)}</GODOWNNAME>
                <BATCHNAME>${xmlEscape(BATCH_DEFAULT)}</BATCHNAME>
                <AMOUNT>${negAmt}</AMOUNT>
                <ACTUALQTY>${qtyStr}</ACTUALQTY>
                <BILLEDQTY>${qtyStr}</BILLEDQTY>
              </BATCHALLOCATIONS.LIST>
...
// Ledger line for the item-invoice purchase path, in Tally's invoice-mode signs.
// Debits (GST, non-stock expense lines): NEGATIVE + ISDEEMEDPOSITIVE Yes.
// Party credit: POSITIVE + ISDEEMEDPOSITIVE No + ISPARTYLEDGER, with a New Ref bill.
// (Distinct from entry(): that uses the accounting-voucher convention for the
//  legacy non-stock purchase path and for freight, which both already work.)
function invModeEntry(ledgerName, amount, opts = {}) {
  const { isParty = false, billRef = null } = opts;
  const abs = Math.abs(n(amount)).toFixed(2);
  if (isParty) {
    const bill = billRef ? `
              <BILLALLOCATIONS.LIST>
                <NAME>${xmlEscape(billRef)}</NAME>
...
function buildPurchaseXML(inv) {
  ...
  // A purchase carrying stock items is a goods purchase → build it as a Tally
  // ITEM INVOICE (the Name-of-Item / Qty / Rate layout), using the exact
  // structure + sign convention proven by live test-push. A purchase with no
  // stock items (legacy / pure-expense) keeps the original accounting-voucher
  // path below, which already works.
  const hasStock = items.some(it => it.kind === 'stock' && it.stock_item_name);
  ...
  if (hasStock) {
    ...
    for (const it of items) {
      ...
      if (it.kind === 'stock' && it.stock_item_name) {
        // each stock line debits its own purchase ledger (falls back to the
        // invoice-level purchaseLedger when the line doesn't specify one)
        const accLedger = it.ledger || purchaseLedger;
        lineEntries += inventoryEntry(it.stock_item_name, it.qty, it.rate, amt, accLedger, it.uom);
      } else {
        // a non-stock line sharing a goods invoice → invoice-mode ledger debit
        const ledger = it.ledger || resolveLedger(it.description || '');
        lineEntries += invModeEntry(ledger, amt);
      }
      linesSum += amt;
    }
    const net = linesSum + cgst + sgst + igst;
    const gstEntries =
      (cgst > 0 ? invModeEntry(CGST_LEDGER, cgst) : '') +
      (sgst > 0 ? invModeEntry(SGST_LEDGER, sgst) : '') +
      (igst > 0 ? invModeEntry(IGST_LEDGER, igst) : '');
    const voucher = `<TALLYMESSAGE xmlns:UDF="TallyUDF">
          <VOUCHER VCHTYPE="Purchase" ACTION="Create"${invoiceNo ? ` REMOTEID="${invoiceNo}"` : ''}>
            ...
            ${lineEntries}${gstEntries}
            ${invModeEntry(party, net, { isParty: true, billRef: invoiceNo || null })}
          </VOUCHER>
        </TALLYMESSAGE>`;
    return envelope(voucher);
  }

  // ── Legacy / pure-expense purchase: accounting-voucher mode (unchanged) ──
  // Back-compat: if no line has a 'kind' field, fall back to a single aggregate
  // purchase-ledger entry using taxable_value.
  let lineEntries = '';
  let linesSum = 0;
